...
// Full Alumni Management — Admin view (tabs: Alumni | Events | Posts | Mentorship)
  ...
  <div className="fixed bottom-6 right-6 z-[100] flex flex-col gap-2 pointer-events-none">
    <AnimatePresence>
      {toasts.map((t) => (
        ...
        >
          {t.type === "success" ? <CheckCircle2 size={16} /> : t.type === "error" ? <XCircle size={16} /> : <AlertCircle size={16} />}
          {t.message}
        </motion.div>
      ))}
    </AnimatePresence>
...
export default function AlumniManagement() {
  ...
  const [addForm, setAddForm] = useState({
    ...
    employmentStatus: "EMPLOYED", currentOrganization: "", designation: "",
    currentCity: "", currentCountry: "", isMentor: false,
  });
  ...
  const loadStats = useCallback(async () => {
    try {
      const res = await getDashboardStats(schoolId);
      setStats(res.data);
    } catch {
      // silently fail stats
    }
  ...
  const loadPassoutYears = useCallback(async () => {
    try {
      const res = await getPassoutYears(schoolId);
      setPassoutYears(res.data || []);
    } catch {}
  }, [schoolId]);
  ...
  const loadEvents = useCallback(async () => {
    ...
    try {
      const res = await getEvents(schoolId);
      setEvents(res.data || []);
    } catch {
  ...
  const loadPosts = useCallback(async () => {
    ...
    try {
      const [approvedRes, pendingRes] = await Promise.all([
        getApprovedPosts(schoolId),
        getPendingPosts(schoolId),
      ]);
      setPosts(approvedRes.data || []);
  ...
  const loadMentors = useCallback(async () => {
    ...
    try {
      const res = await getMentors(schoolId);
      setMentors(res.data || []);
    } catch {
  ...
  const handleAddAlumni = async (e) => {
    ...
    try {
      ...
      setAddModal(false);
      setAddForm({ firstName: "", lastName: "", email: "", phoneNumber: "", passoutYear: "", lastClass: "", admissionNumber: "", employmentStatus: "EMPLOYED", currentOrganization: "", designation: "", currentCity: "", currentCountry: "", isMentor: false });
      loadAlumni();
      loadStats();
    } catch (e) {
      toast(e?.response?.data?.message || "Failed to add alumni", "error");
    } finally {
      setAddLoading(false);
    }
  };
  ...
  const handleCreateEvent = async (e) => {
    ...
    try {
      ...
      setEventModal(false);
      setEventForm({ title: "", description: "", eventType: "REUNION", eventDate: "", eventTime: "", venue: "", meetLink: "", registrationDeadline: "", maxParticipants: "", isPublic: true });
      loadEvents();
      loadStats();
    } catch (e) {
      toast(e?.response?.data?.message || "Failed to create event", "error");
    } finally {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
                  ...
                  {events.map((ev) => (
                    <motion.div key={ev.id} layout
                      ...
                      <div className="flex items-start justify-between gap-2 mb-3">
                        ...
                        {ev.isPublic && (
                          <span className="text-xs bg-green-50 text-green-600 px-2 py-0.5 rounded-md flex-shrink-0">Public</span>
                        )}
                      </div>
                      ...
                      {m.mentorSkills && (
                        <div className="flex flex-wrap gap-1">
                          {m.mentorSkills.split(",").slice(0, 4).map((skill, i) => (
                            <span key={i} className="text-xs bg-purple-50 text-purple-600 px-2 py-0.5 rounded-md">
                              {skill.trim()}
                            </span>
                          ))}
                        </div>
      ...
      {/* ─── Add Alumni Modal ─────────────────────────────────────────────── */}
      <Modal open={addModal} onClose={() => setAddModal(false)} title="Add Alumni Manually" wide>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Email">
              <input type="email" value={addForm.email} onChange={(e) => setAddForm((p) => ({ ...p, email: e.target.value }))}
                className={inp} placeholder="rahul@gmail.com" />
            </Field>
            <Field label="Phone">
              <input value={addForm.phoneNumber} onChange={(e) => setAddForm((p) => ({ ...p, phoneNumber: e.target.value }))}
                className={inp} placeholder="+91 9999..." />
            </Field>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <Field label="Passout Year *">
              <input required value={addForm.passoutYear} onChange={(e) => setAddForm((p) => ({ ...p, passoutYear: e.target.value }))}
                className={inp} placeholder="2023" />
            </Field>
            <Field label="Last Class">
              <input value={addForm.lastClass} onChange={(e) => setAddForm((p) => ({ ...p, lastClass: e.target.value }))}
                className={inp} placeholder="Class 12 - Science" />
            </Field>
          </div>

          <Field label="Admission Number">
            <input value={addForm.admissionNumber} onChange={(e) => setAddForm((p) => ({ ...p, admissionNumber: e.target.value }))}
              className={inp} placeholder="SCH-2019-042 (links to existing student)" />
          </Field>

          <div className="grid grid-cols-2 gap-3">
            <Field label="Employment Status">
              <select value={addForm.employmentStatus} onChange={(e) => setAddForm((p) => ({ ...p, employmentStatus: e.target.value }))}
                className={sel}>
                {EMPLOYMENT_STATUS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
              </select>
            </Field>
            <Field label="Current Organization">
              <input value={addForm.currentOrganization} onChange={(e) => setAddForm((p) => ({ ...p, currentOrganization: e.target.value }))}
                className={inp} placeholder="Google India" />
            </Field>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Designation">
              <input value={addForm.designation} onChange={(e) => setAddForm((p) => ({ ...p, designation: e.target.value }))}
                className={inp} placeholder="Software Engineer" />
            </Field>
            <Field label="Current City">
              <input value={addForm.currentCity} onChange={(e) => setAddForm((p) => ({ ...p, currentCity: e.target.value }))}
                className={inp} placeholder="Mumbai" />
            </Field>
          </div>

          <label className="flex items-center gap-2 cursor-pointer select-none">
            <input type="checkbox" checked={addForm.isMentor} onChange={(e) => setAddForm((p) => ({ ...p, isMentor: e.target.checked }))}
      ...
      {/* ─── Create Event Modal ───────────────────────────────────────────── */}
      <Modal open={eventModal} onClose={() => setEventModal(false)} title="Create Alumni Event" wide>
        <form onSubmit={handleCreateEvent} className="space-y-4">
          <Field label="Event Title *">
            <input required value={eventForm.title} onChange={(e) => setEventForm((p) => ({ ...p, title: e.target.value }))}
              className={inp} placeholder="Annual Alumni Meet 2025" />
          </Field>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Event Date *">
              <input required type="date" value={eventForm.eventDate} onChange={(e) => setEventForm((p) => ({ ...p, eventDate: e.target.value }))}
                className={inp} />
            </Field>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Event Time">
              <input type="time" value={eventForm.eventTime} onChange={(e) => setEventForm((p) => ({ ...p, eventTime: e.target.value }))}
                className={inp} />
            </Field>
            <Field label="Registration Deadline">
              <input type="date" value={eventForm.registrationDeadline} onChange={(e) => setEventForm((p) => ({ ...p, registrationDeadline: e.target.value }))}
                className={inp} />
            </Field>
          </div>
          ...
          <Field label="Venue">
            <input value={eventForm.venue} onChange={(e) => setEventForm((p) => ({ ...p, venue: e.target.value }))}
              className={inp} placeholder="School Auditorium, Mumbai" />
          </Field>

          <Field label="Meet Link (for online events)">
            <input type="url" value={eventForm.meetLink} onChange={(e) => setEventForm((p) => ({ ...p, meetLink: e.target.value }))}
              className={inp} placeholder="https://meet.google.com/..." />
          </Field>

          <div className="grid grid-cols-2 gap-3">
            <Field label="Max Participants">
              <input type="number" min="1" value={eventForm.maxParticipants} onChange={(e) => setEventForm((p) => ({ ...p, maxParticipants: e.target.value }))}
                className={inp} placeholder="200" />
            </Field>
            <div className="flex items-end pb-0.5">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={eventForm.isPublic} onChange={(e) => setEventForm((p) => ({ ...p, isPublic: e.target.checked }))}
                  className="w-4 h-4 rounded accent-blue-600" />
          ...
          <Field label="Description">
            <textarea rows={3} value={eventForm.description} onChange={(e) => setEventForm((p) => ({ ...p, description: e.target.value }))}
              className={`${inp} resize-none`} placeholder="Event description…" />
          </Field>

          <div className="flex gap-2 pt-2">
            <button type="button" onClick={() => setEventModal(false)}
              className="flex-1 py-2.5 border border-gray-200 text-gray-600 font-semibold rounded-xl hover:bg-gray-50 transition-colors text-sm">
              Cancel
            </button>
