...
// // src/school_admin/pages/Exams.jsx
// // Role: SCHOOL_ADMIN, PRINCIPAL
// // Tabs: Exams (CRUD) | Schedule | Results | Rank List | Report Card | Bulk Upload
// // All APIs wired to exact endpoints from ExamController, ExamScheduleController, ResultController
...
//       color: "#fff", padding: "12px 22px", borderRadius: 10,
//       fontWeight: 600, zIndex: 9999, boxShadow: "0 4px 20px rgba(0,0,0,0.2)",
//       fontSize: 14, animation: "slideIn 0.2s ease",
//     }}>
//       {msg}
//     </div>
//   );
...
// const EXAM_TYPES = ["UNIT_TEST", "MID_TERM", "FINAL", "ANNUAL", "QUARTERLY", "HALF_YEARLY"];
// const BLANK_EXAM = { examName: "", examType: "UNIT_TEST", examTypeString: "", standard: "", startDate: "", endDate: "", academicYear: "2026-2027" };
// const BLANK_SLOT = { subject: "", examDate: "", startTime: "09:00", endTime: "12:00" };
...
//   // ── EXAM CRUD ─────────────────────────────────────────────────────────────
//   const openCreate = () => { setExamForm(BLANK_EXAM); setEditingExamId(null); setShowExamModal(true); };
//   const openEdit = (ex) => { setExamForm({ ...ex }); setEditingExamId(ex.id); setShowExamModal(true); };

//   const submitExam = () => wrap(async () => {
//     const { examName, standard, startDate, endDate } = examForm;
//     if (!examName || !standard || !startDate || !endDate)
//       return notify("Please fill all required fields", false);
//     try {
...
//       await refreshExams();
//     } catch (e) {
//       notify(e.response?.data?.message || "Operation failed", false);
//     }
//   });
...
//   // ── RANK LIST ─────────────────────────────────────────────────────────────
//   // Uses GET /result/class — same endpoint, backend returns ranked data
//   const loadRankList = () => wrap(async () => {
//     if (!selectedExamId) return notify("Select an exam first", false);
//     try {
//       const r = await resultService.getByClass(selectedExamId);
...
//   // ── REPORT CARD ───────────────────────────────────────────────────────────
//   // Uses GET /result/student?studentId=&examId=
//   const loadReportCard = () => wrap(async () => {
//     if (!selectedExamId || !rcStudentId)
//       return notify("Select exam & enter student ID", false);
//     try {
...
//         <div style={{ marginBottom: 24 }}>
//           <h1 style={{ margin: 0, fontSize: 26, fontWeight: 800, color: "#1e293b" }}>Exam Management</h1>
//           <p style={{ margin: "4px 0 0", color: "#64748b", fontSize: 14 }}>Create exams, manage schedules, enter & review results</p>
//         </div>
...
//               </thead>
//               <tbody>
//                 {exams.length === 0 && <Empty text='No exams yet. Click "Create Exam" to get started.' />}
//                 {exams.map((ex) => (
...
//                   <RefreshCcw size={14} /> {loading ? "Loading..." : "Load"}
//                 </button>
//                 <button onClick={() => { setSlotForm(BLANK_SLOT); setShowSlotModal(true); }}
//                   disabled={!selectedExamId} style={S.btn()}>
//                   <Plus size={14} /> Add Slot
//                 </button>
//               </div>
...
//                   <tbody>
//                     {rankList.students.map((r) => (
//                       <tr key={r.id || r._rank} style={{ borderBottom: "1px solid #f8fafc", background: r._rank <= 3 ? "#fffbeb" : "transparent" }}>
//                         <td style={{ ...S.td, fontWeight: 800, fontSize: 16 }}>
//                           {r._rank === 1 ? "🥇" : r._rank === 2 ? "🥈" : r._rank === 3 ? "🥉" : `#${r._rank}`}
//                         </td>
//                         <td style={{ ...S.td, fontWeight: 600 }}>
...
//             {reportCard && (
//               <div id="reportCardPrint" style={{ background: "#fff", borderRadius: 14, padding: 32, boxShadow: "0 1px 8px rgba(0,0,0,0.07)" }}>
//                 {/* Header */}
//                 <div style={{ textAlign: "center", borderBottom: "2px solid #4f46e5", paddingBottom: 16, marginBottom: 20 }}>
//                   <h2 style={{ margin: 0, fontSize: 22, fontWeight: 800, color: "#1e293b" }}>
//                     {reportCard.schoolName || "School Name"}
...
//                 desc: "Create exam + schedule via Excel (.xlsx). Required columns: examName, examType, standard, subject, examDate, startTime, endTime",
//                 color: "#059669",
//                 icon: <Calendar size={40} color="#059669" />,
//                 note: "Creates the exam and all schedule slots in one go.",
//               },
//             ].map((u) => (
//               <div key={u.type} style={{ background: "#fff", borderRadius: 14, padding: 28, boxShadow: "0 1px 8px rgba(0,0,0,0.07)", textAlign: "center" }}>
//                 <div style={{ marginBottom: 12 }}>{u.icon}</div>
...
//       {showExamModal && (
//         <Modal title={editingExamId ? "Edit Exam" : "Create New Exam"} onClose={() => setShowExamModal(false)}>
//           <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 12px" }}>
//             <Field label="Exam Name *">
//               <input style={S.inp} value={examForm.examName}
//                 onChange={(e) => setExamForm((p) => ({ ...p, examName: e.target.value }))}
...
//               style={{ ...S.btn(), flex: 1, justifyContent: "center" }}>
//               <CheckCircle size={14} /> {loading ? "Saving..." : editingExamId ? "Update Exam" : "Create Exam"}
//             </button>
//             <button onClick={() => setShowExamModal(false)} style={{ ...S.btn("#94a3b8"), flex: 1, justifyContent: "center" }}>
//               Cancel
//             </button>
...
//       {showSlotModal && (
//         <Modal title="Add Schedule Slot" onClose={() => setShowSlotModal(false)}>
//           <Field label="Subject *">
//             <input style={S.inp} value={slotForm.subject}
//               onChange={(e) => setSlotForm((p) => ({ ...p, subject: e.target.value }))}
//               placeholder="e.g. Mathematics" />
