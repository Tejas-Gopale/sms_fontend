...
import {
  ...
  Banknote   // ✅ FIX: was missing from imports, caused crash in payment modal
} from "lucide-react";
...
export default function FeeManagement() {
  ...
  const handleSubmitStructure = async () => {
    ...
    try {
      ...
      // Step 2 – add all items in parallel
      await Promise.all(
  ...
  const handleGenerateClassFees = async () => {
    ...
    try {
      ...
      // ✅ Show the actual backend error message (e.g. "structure not found")
  ...
  const handleProcessPayment = async () => {
    ...
    try {
      ...
      await API.post("/fees/pay", {
        // ✅ FIX: studentFeeId now comes correctly from the API response
