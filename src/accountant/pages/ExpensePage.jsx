...
import {
  Receipt, Plus, Search, RefreshCw, Loader2,
...
const blankForm = {
  ...
  description: "", vendorName: "", billNumber: "", academicYear: ""
};
...
function ExpenseForm({ initial = {}, onSubmit, onCancel, loading }) {
  ...
  const handleSubmit = () => {
    if (!form.title || !form.category || !form.amount || !form.expenseDate || !form.academicYear) return;
    onSubmit(form);
  };
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        ...
        <FieldInput label="Academic Year" value={form.academicYear} onChange={set("academicYear")} options={ACADEMIC_YEARS} required />
        <FieldInput label="Vendor Name" value={form.vendorName} onChange={set("vendorName")} placeholder="Optional" />
        <FieldInput label="Bill Number" value={form.billNumber} onChange={set("billNumber")} placeholder="Optional" />
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button
          ...
          disabled={loading || !form.title || !form.category || !form.amount || !form.expenseDate || !form.academicYear}
...
function ExpenseDetail({ expense, onClose, onApprove, onReject, isAdmin, approving, rejecting }) {
  ...
  const rows = [
    ...
    ["Academic Year",  expense.academicYear || "—"],
    ["Vendor",         expense.vendorName || "—"],
    ["Bill No.",       expense.billNumber || "—"],
...
export default function ExpensePage() {
  ...
  const userRole    = (userData?.roles && userData.roles[0]) || userData?.role || "";
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <div className="p-8 space-y-6">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
            {[
              { icon: Receipt, label: "Total Expenses", value: String(expenses.length), sub: "All recorded", color: "yellow" },
              { icon: IndianRupee, label: "Total Amount", value: fmtINR(totalAmount), sub: "Across all statuses", color: "blue" },
              { icon: CheckCircle, label: "Approved Amount", value: fmtINR(approvedTotal), sub: "Approved expenses", color: "green" },
              { icon: Clock, label: "Pending Approvals", value: String(pendingCount), sub: "Awaiting review", color: pendingCount > 0 ? "red" : "gray" },
            ].map((c) => (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50/60">
                    {["#", "Title", "Category", "Amount", "Date", "Vendor", "Added By", "Status", "Actions"].map((h) => (
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={9} className="px-5 py-14 text-center text-gray-400">
        ...
        <Modal title="Edit Expense" onClose={() => setEditExpense(null)} wide>
          <ExpenseForm
            initial={{
              ...
              academicYear: editExpense.academicYear || "",
              description:  editExpense.description  || "",
              vendorName:   editExpense.vendorName   || "",
              billNumber:   editExpense.billNumber   || "",
            }}
