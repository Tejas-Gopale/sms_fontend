...
function Section({ icon: Icon, title, subtitle, children, accent = "blue" }) {
  const accents = {
    ...
    red: "from-red-500 to-red-600", slate: "from-slate-500 to-slate-600",
  };
  return (
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        <div className="bg-white border-b border-gray-200 px-8 py-5">
          <div className="flex items-center gap-2 text-xs text-gray-400 mb-1">
            <span>Teacher Panel</span><ChevronRight size={12} />
...
// ═════════════════════════════════════════════
// TAB: PROFILE
// Backend: GET /user/profile  →  UserProfileResponse
//          PUT /user/profile  →  UserProfileRequest
// ═════════════════════════════════════════════
function ProfileTab({ addToast }) {
  const userData = getUserData();
  const isClassTeacher = userData?.roles?.includes("CLASS_TEACHER");
  ...
  const [form, setForm] = useState({
    ...
    dateOfBirth:     "",
    emergencyContact: "",
    // Address fields (backend splits address into components)
    addressLine1:    "",
    addressLine2:    "",
    city:            "",
    state:           "",
  ...
  const save = async () => {
    ...
    try {
      await API.put("/user/profile", {
        ...
        bio:              form.bio,
        // Backend expects LocalDate in yyyy-MM-dd — HTML date input already gives this
  ...
  const initials = form.fullName.split(" ").map((n) => n[0]).join("").toUpperCase().slice(0, 2) || "T";
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-6 py-5 flex items-center gap-5">
        ...
        <div>
          <p className="font-semibold text-gray-900">{form.fullName || "Teacher"}</p>
          ...
          {isClassTeacher && (
            <span className="inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-700">
              <GraduationCap size={9} /> Class Teacher
            </span>
          )}
        </div>
        <div className="ml-auto text-right">
          <p className="text-xs text-gray-400">Role</p>
          <p className="text-sm font-semibold text-gray-700">{isClassTeacher ? "Class Teacher" : "Teacher"}</p>
        </div>
      </div>
          ...
          <Section icon={User} title="Personal Information" subtitle="Your basic profile details" accent="blue">
            <div className="grid grid-cols-2 gap-4">
              ...
              <Field label="Designation">
                <Input name="designation" value={form.designation} onChange={handle} placeholder="e.g. Senior Teacher" />
              </Field>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-800">
                      ...
                      {s.current && (
                        <span className="ml-2 px-1.5 py-0.5 bg-emerald-100 text-emerald-700 text-[10px] font-bold rounded-full">Current</span>
                      )}
                    </p>
...
// ═════════════════════════════════════════════
// TAB: NOTIFICATIONS
// Backend: GET /user/notification-preferences
//          PUT /user/notification-preferences
// ═════════════════════════════════════════════
function NotificationsTab({ addToast }) {
  const defaultPrefs = {
    emailAttendance: true,  emailLeave: true,  emailExam: true,
    emailSalary: true,      emailNewsletter: false,
    pushAttendance: true,   pushLeave: true,   pushExam: false,   pushSalary: false,
    inAppAll: true,         inAppHomework: true, inAppRemarks: true,
  };
  ...
  return (
    <div className="space-y-6">
      <Section icon={Mail} title="Email Notifications" subtitle="Receive alerts in your inbox" accent="blue">
        <Toggle checked={prefs.emailAttendance}  onChange={() => toggle("emailAttendance")}  label="Attendance Alerts"       sub="Notified when students are absent or late" />
        ...
        <Toggle checked={prefs.emailExam}         onChange={() => toggle("emailExam")}         label="Exam & Test Reminders"    sub="Upcoming exams and result uploads" />
        <Toggle checked={prefs.emailSalary}       onChange={() => toggle("emailSalary")}       label="Salary Disbursement"      sub="Notified when salary is credited" />
      ...
      <Section icon={Bell} title="Push Notifications" subtitle="Browser and mobile push alerts" accent="violet">
        <Toggle checked={prefs.pushAttendance} onChange={() => toggle("pushAttendance")} label="Attendance"   sub="Real-time alerts for attendance events" />
        <Toggle checked={prefs.pushLeave}      onChange={() => toggle("pushLeave")}      label="Leave Updates" sub="Instant notification on leave status" />
        <Toggle checked={prefs.pushExam}       onChange={() => toggle("pushExam")}       label="Exams & Tests" sub="Reminders before exam dates" />
        <Toggle checked={prefs.pushSalary}     onChange={() => toggle("pushSalary")}     label="Salary Alerts" sub="Push alert on salary credit" />
      </Section>
      <Section icon={Bell} title="In-App Notifications" subtitle="Alerts inside the dashboard" accent="emerald">
        ...
        <Toggle checked={prefs.inAppHomework} onChange={() => toggle("inAppHomework")} label="Homework Submissions"  sub="When students submit homework" />
        <Toggle checked={prefs.inAppRemarks}  onChange={() => toggle("inAppRemarks")}  label="Remark Confirmations"  sub="When student remarks are saved" />
      </Section>
...
// ═════════════════════════════════════════════
// TAB: PREFERENCES
// Backend: GET /user/preferences   →  UserPreferencesResponse
//          PUT /user/preferences   →  UserPreferencesRequest
// Note: backend uses defaultPageSize (Integer), frontend used defaultAttendancePage (string)
// ═════════════════════════════════════════════
function PreferencesTab({ addToast }) {
  const defaultPrefs = {
    theme: "light", language: "en", timezone: "Asia/Kolkata",
    dateFormat: "DD/MM/YYYY", timeFormat: "12h",
    compactMode: false, showRollNumbers: true, defaultPageSize: 20,
  };
  const [prefs, setPrefs] = useState(defaultPrefs);
  const [loading, setLoading] = useState(true);
  ...
  const save = async () => {
    try {
      // Convert defaultPageSize to Integer for backend
      await API.put("/user/preferences", {
        ...prefs,
        defaultPageSize: parseInt(prefs.defaultPageSize, 10),
      });
  ...
  return (
    <div className="space-y-6">
      ...
      <Section icon={Clock} title="Dashboard Defaults" subtitle="Default views and pagination" accent="slate">
        <Field label="Default Attendance List Size" hint="How many records to show per page">
          {/* Backend field name: defaultPageSize (Integer) */}
          <Select value={String(prefs.defaultPageSize)} onChange={(v) => set("defaultPageSize", parseInt(v, 10))}
            options={[{ value: "10", label: "10 per page" }, { value: "20", label: "20 per page" }, { value: "50", label: "50 per page" }, { value: "100", label: "100 per page" }]} />
        </Field>
        <div className="mt-3">
          <Toggle checked={prefs.showRollNumbers} onChange={(v) => set("showRollNumbers", v)} label="Show Roll Numbers" sub="Display roll numbers alongside student names" />
        </div>
      </Section>
...
// ═════════════════════════════════════════════
// TAB: PRIVACY
// Backend: GET /user/privacy-settings
//          PUT /user/privacy-settings
// ═════════════════════════════════════════════
function PrivacyTab({ addToast }) {
  const defaultPrefs = {
    showPhoneToStudents: false, showPhoneToParents: true,
    showEmailToStudents: false, showEmailToParents: false,
    allowParentMessages: true,  showOnlineStatus: true, activityLog: true,
  };
  const [prefs, setPrefs] = useState(defaultPrefs);
  const [loading, setLoading] = useState(true);
  ...
  return (
    <div className="space-y-6">
      <Section icon={Shield} title="Contact Visibility" subtitle="Who can see your contact information" accent="violet">
        <Toggle checked={prefs.showPhoneToStudents} onChange={() => toggle("showPhoneToStudents")} label="Show Phone to Students" sub="Students can see your phone number" />
        <Toggle checked={prefs.showPhoneToParents}  onChange={() => toggle("showPhoneToParents")}  label="Show Phone to Parents"  sub="Parents can see your phone number" />
        <Toggle checked={prefs.showEmailToStudents} onChange={() => toggle("showEmailToStudents")} label="Show Email to Students" sub="Students can see your email address" />
        <Toggle checked={prefs.showEmailToParents}  onChange={() => toggle("showEmailToParents")}  label="Show Email to Parents"  sub="Parents can see your email address" />
      </Section>
      <Section icon={Bell} title="Communication Preferences" subtitle="Control how others can reach you" accent="blue">
        <Toggle checked={prefs.allowParentMessages} onChange={() => toggle("allowParentMessages")} label="Allow Parent Messages" sub="Parents can send you messages via the portal" />
      ...
      <Section icon={Download} title="Data & Activity" subtitle="Your data and how it is used" accent="slate">
        <div className="flex items-center justify-between py-2">
          <div>
            ...
            <p className="text-xs text-gray-400">Export your profile, attendance, and activity data</p>
          </div>
          <button
            onClick={async () => {
              try {
                ...
                addToast("Failed to submit export request", "error");
              }
            }}
...
// ═════════════════════════════════════════════
// TAB: ACCOUNT
// Backend: POST /auth/logout-all  (existing)
//          GET  /user/profile     (reuse for account info)
// ═════════════════════════════════════════════
function AccountTab({ addToast }) {
  ...
  const handleLogoutAll = async () => {
    const refreshToken = localStorage.getItem("refreshToken") || "";
    try {
      // Uses DELETE /user/sessions/all-others with current refresh token
      await API.delete("/user/sessions/all-others", { headers: { "X-Refresh-Token": refreshToken } });
      addToast("Signed out from all other devices");
    } catch {
      addToast("Failed to sign out all devices", "error");
    }
  };
  ...
  return (
    <div className="space-y-6">
      <Section icon={User} title="Account Information" subtitle="Your account details" accent="blue">
        <div className="grid grid-cols-2 gap-4">
          {[
            { label: "Account Type", value: userData?.roles?.join(", ") || "Teacher" },
...
//           </span>
//         ))}
//         <span className={`text-xs font-semibold ml-auto ${color.replace("bg-", "text-")}`}>
//           {label}
//         </span>
//       </div>
//     </div>
...
//               <div className="col-span-2">
//                 <Field label="Address">
//                   <Input name="address" value={form.address} onChange={handle} placeholder="Your address" />
//                 </Field>
//               </div>
//               <div className="col-span-2">
//                 <Field label="Bio / About" hint="Brief description visible on your profile">
...
//     { id: 3, device: "Chrome on MacBook", location: "Pune, IN",   time: "Yesterday",    current: false },
//   ]);
...
//     setSaving(true);
//     try {
//       await API.put("/user/change-password", { currentPassword: pw.current, newPassword: pw.newPw });
//       addToast("Password changed successfully");
//       setPw({ current: "", newPw: "", confirm: "" });
//     } catch (err) {
//       addToast(err.response?.data?.message || "Failed to change password", "error");
...
//             <PasswordInput name="current" placeholder="Enter current password" />
//           </Field>
//           <Field label="New Password">
//             <PasswordInput name="newPw" placeholder="Enter new password" />
//             <PasswordStrength password={pw.newPw} />
...
//                     {s.device}
//                     {s.current && (
//                       <span className="ml-2 px-1.5 py-0.5 bg-emerald-100 text-emerald-700 text-[10px] font-bold rounded-full">
//                         Current
//                       </span>
//                     )}
//                   </p>
...
//               {!s.current && (
//                 <button
//                   onClick={() => addToast("Session revoked")}
//                   className="text-xs text-red-500 hover:text-red-700 font-medium hover:underline"
//                 >
//                   Revoke
//                 </button>
...
//         </div>
//         <button
//           onClick={() => addToast("All other sessions revoked")}
//           className="mt-4 text-xs text-red-500 hover:text-red-700 font-semibold flex items-center gap-1 hover:underline"
//         >
//           <LogOut size={12} /> Sign out all other sessions
//         </button>
...
//             label="Compact Mode"
//             sub="Reduces spacing for more information density"
//           />
//         </div>
//       </Section>
...
//                 { value: "Asia/Mumbai",   label: "IST — Mumbai" },
//                 { value: "UTC",           label: "UTC" },
//               ]}
//             />
//           </Field>
...
//         <Field label="Default Attendance List Size" hint="How many records to show per page">
//           <Select
//             name="defaultAttendancePage"
//             value={prefs.defaultAttendancePage}
//             onChange={(v) => set("defaultAttendancePage", v)}
//             options={[
...
//         <div className="mt-3">
//           <Toggle
//             checked={prefs.showRollNumbers}
//             onChange={(v) => set("showRollNumbers", v)}
//             label="Show Roll Numbers"
//             sub="Display roll numbers alongside student names"
//           />
//         </div>
//       </Section>
...
//     showPhoneToParents:  true,
//     showEmailToStudents: false,
//     showEmailToParents:  false,
...
//             </div>
//             <button
//               onClick={() => addToast("Data export request submitted. You'll receive an email.", "info")}
//               className="flex items-center gap-1.5 text-xs text-blue-600 font-semibold border border-blue-200 px-3 py-1.5 rounded-lg hover:bg-blue-50 transition-colors"
//             >
//               <Download size={12} /> Export
//             </button>
...
//       addToast("Signed out from all devices");
//     } catch {
//       addToast("Failed to sign out all devices", "error");
//     }
//   };
...
//               <p className="text-xs text-gray-400">Log out from every device where you're signed in</p>
//             </div>
//             <button
...
//                 Temporarily disable your account. Contact the School Admin to reactivate.
//               </p>
//             </div>
//             <button
...
//               <div className="flex gap-2">
//                 <button
//                   disabled={deleteText !== "DEACTIVATE"}
//                   onClick={() => addToast("Deactivation request sent to admin", "info")}
//                   className="flex-1 text-xs bg-red-600 hover:bg-red-700 disabled:opacity-40 disabled:cursor-not-allowed text-white py-2 rounded-lg font-semibold transition-all"
//                 >
//                   Confirm Deactivate
