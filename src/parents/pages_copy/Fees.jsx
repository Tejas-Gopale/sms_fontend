          ...
          <>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 text-center">
                ...
                {fees.upiId ? (
                  <>
                    <p className="text-sm text-gray-500 mb-4">Scan QR Code to Pay via UPI</p>
                    <img
                      src={
                        fees.paymentQRCode
                          ? fees.paymentQRCode
                          : `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=upi://pay?pa=${encodeURIComponent(fees.upiId)}&pn=SchoolFees&am=${fees.remainingAmount ?? ""}&cu=INR`
                      }
                      alt="UPI QR Code"
                      className="mx-auto w-44 h-44 object-contain"
                    />
                    <p className="mt-3 text-xs text-gray-400">PhonePe / Google Pay / Paytm / Any UPI</p>
                    <p className="mt-1 text-sm font-semibold text-indigo-600">{fees.upiId}</p>
                  </>
                ) : (
                  <div className="flex flex-col items-center justify-center py-8 text-gray-400">
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-14 h-14 mb-3 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 4v1m0 14v1M4.22 4.22l.707.707M18.364 18.364l.707.707M1 12h1m20 0h1M4.22 19.778l.707-.707M18.364 5.636l.707-.707" />
                    </svg>
                    <p className="text-sm font-medium text-gray-500">Online payment not configured</p>
                    <p className="text-xs text-gray-400 mt-1">Please pay at the school office or contact admin</p>
                  </div>
                )}
                <p className="mt-3 text-xs text-gray-300">UPI / PhonePe / Google Pay</p>
              </div>
